...
import { FavoriteProduct, FavoriteProductSchema } from "src/schema/favourites.schema";
import { UserModule } from "../users/users.module";
import { ProductModel } from "../products/products.module";
import { User, UserSchema } from "src/schema/user.schema";
@Module({
    imports:[MongooseModule.forFeature([{name:FavoriteProduct.name,schema:FavoriteProductSchema}]),
    MongooseModule.forFeature([{name:User.name,schema:UserSchema}]),
    MongooseModule.forFeature([{name:Product.name,schema:ProductSchema}]),
    forwardRef(()=>RefreshTokenModule),
    forwardRef(()=>AuthModule),
    forwardRef(()=>PermissionModule),
    forwardRef(()=>RoleModule),UserModule,ProductModel
