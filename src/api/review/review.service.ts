...
export class ReviewService {
  ...
  async updateReview(id: string, updateReviewRequest: UpdateReviewRequest): Promise<Review> {
    const review = await this.reviewModel.findByIdAndUpdate(id, updateReviewRequest, { new: true }).exec();
    if (!review) {
      throw new NotFoundException(`Review with ID ${id} not found`);
    }
    return review;
