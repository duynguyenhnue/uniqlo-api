...
import { Warehouse } from "../../schema/warehouse.shema";
...
export class WarehouseService {
  ...
  private async createwarehouseindb(
    createWarehouseRequest: CreateWarehouseRequest
  ): Promise<Warehouse> {
  ...
  async search(
    ...
    const data = await this.WarehouseModel.find(filter)
      .sort({ createdAt: -1 })
      .skip(offset)
      .limit(limit)
      .exec();
  ...
  async findAll(): Promise<WarehouseResponse[]> {
    ...
    return warehouse.map((warehouses) =>
      this.mapWarehouseToResponse(warehouses)
    );
  }
  ...
  async update(
    ...
    console.log("ID:", id);
    const update = await this.WarehouseModel.findByIdAndUpdate(
      id,
      updateWarehouseRequest,
      { new: true }
    ).exec();
