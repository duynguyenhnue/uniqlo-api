...
async createOrder(userId:string,create:CreateOrderRequest):Promise<OrderRespone>{
    ...
    const products = await this.productModel.find({ _id: { $in: orderItems.map(item => item.productId) } }).exec();
    return this.mapOrderToResponse(order,user,products);

    }catch(error)
    {
        throw new BadRequestException(`Error while create order ${error.message}`);
    }
}
...
async search(query:SearchOrderbyIdRequest):Promise<{data:OrderRespone[];total:number}>
{
    ...
    if(orderId)
      ...
      const userIds = [...new Set(data.map(order => order.userId.toString()))];
        const users = await this.userModel.find({ _id: { $in: userIds } }).exec(); 

        const productIds = [...new Set(data.flatMap(order => order.orderItems.map(item => item.productId.toString())))];
        const products = await this.productModel.find({ _id: { $in: productIds } }).exec();  
      return{
        data:data.map(o=>this.mapOrderToResponse(o, users.find(user => user._id.toString() === o.userId.toString()), products)),
      total,
      };
    }catch(error)
    {
...
async findAll():Promise<OrderRespone[]>{
    try{
        ...
        const userIds = [...new Set(order.map(order => order.userId.toString()))];
        const users = await this.userModel.find({ _id: { $in: userIds } }).exec(); 

        const productIds = [...new Set(order.flatMap(order => order.orderItems.map(item => item.productId.toString())))];
        const products = await this.productModel.find({ _id: { $in: productIds } }).exec();
        return order.map(o=>this.mapOrderToResponse(o,users.find(user=>user._id.toString()===o.userId.toString()),products))
    }catch(error)
    {
        throw new BadRequestException(`Error ${error.message}`);
    }
...
async findOne(id:string):Promise<OrderRespone>
{
    try{
        ...
        if(!order)
            ...
            const products=await this.productModel.find({_id:{$in:order.orderItems.map(o=>o.productId)}}).exec();
...
async updateOrder(userId:string,orderId:string,update:UpdateOrderRequest):Promise<OrderRespone>
{
  ...
  order.totalAmount = totalAmount;
    ...
    const user=await this.userModel.findById(order.userId).exec();
    const products= await this.productModel.find({_id:{$in:order.orderItems.map(i=>i.productId)}}).exec();
    return this.mapOrderToResponse(order,user,products);
...
async updateorderstatus(orderId:string,status:string):Promise<OrderRespone>
{
    ...
    {
        throw new BadRequestException(`Error while update status order ${error.message}`);
    }
}
...
private mapOrderToResponse(order: Order,user:User|null,products:Product[]|null): OrderRespone {
    try{
        return {
          ...
          orderItems:order.orderItems? order.orderItems.map(item => {
            const product = products?.find((p) => p._id.toString() === item.productId.toString());
              return {
                ...
                Product_specifications: product ? product.Product_specifications : null,
                Product_price: product ? product.Product_price : 0,
                Product_count: product ? product.Product_count : 0,
                Product_isNewArrival:product ? product.Product_isNewArrival:false,
                Product_isBestSeller:product ? product.Product_isBestSeller:false,
