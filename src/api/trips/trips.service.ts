...
import { CreateTripsRequest, UpdateTripsRequest } from 'src/payload/request/trips.request';
...
export class TripsService {
  ...
  async updateTrip(id: string, updateTripsRequest: UpdateTripsRequest): Promise<Trip> {
    const trip = await this.tripModel.findByIdAndUpdate(id, updateTripsRequest, { new: true }).exec();
    if (!trip) {
      throw new NotFoundException(`Trip with ID ${id} not found`);
    }
    return trip;
