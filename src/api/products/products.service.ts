...
export class ProductService {
  ...
  async searchproduct(
    ...
    if (minPrice || maxPrice) {
      filter.Product_price = {}; // Khởi tạo object
  
      if (minPrice) {
        filter.Product_price.$gte = Number(minPrice); // Lớn hơn hoặc bằng
      }
  
      if (maxPrice) {
        filter.Product_price.$lte = Number(maxPrice); // Nhỏ hơn hoặc bằng
      }
    }
