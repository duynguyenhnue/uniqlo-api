...
export class PhotoService {
  ...
  async updatePhoto(id: string, updatePhotoRequest: UpdatePhotoRequest): Promise<Photo> {
    const photo = await this.photoModel.findByIdAndUpdate(id, updatePhotoRequest, { new: true }).exec();
    if (!photo) {
      throw new NotFoundException(`Photo with ID ${id} not found`);
    }
    return photo;
