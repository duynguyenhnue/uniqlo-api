...
import { Inventory } from "../../schema/inventories.schema";
...
export class InventoriesService {
  constructor(
    ...
    private readonly InventoriesModel: Model<Inventory>,
  ...
  private async createInventoriesindb(
    createInventoryRequest: CreateInventoryRequest
  ): Promise<Inventory> {
  ...
  async search(
    query: SearchInventoryRequest,
    userId: string
  ): Promise<{ data: InventoryResponse[]; total: number }> {
    ...
    const data = await this.InventoriesModel.find(filter)
      .sort({ createdAt: -1 })
      .skip(offset)
      .limit(limit)
      .exec();
  ...
  async findAll(): Promise<InventoryResponse[]> {
    ...
    return inventory.map((inventories) =>
      this.mapInventoryToResponse(inventories)
    );
  }
  ...
  async update(
    ...
    const update = await this.InventoriesModel.findByIdAndUpdate(
      id,
      updateInventoryRequest,
      { new: true }
    ).exec();
