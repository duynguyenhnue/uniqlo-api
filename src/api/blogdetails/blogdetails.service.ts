...
import { Blogdetails, Comments } from "../../schema/blog-details.schema";
import {
...
export class BlogDetailService {
  constructor(
    @InjectModel(Blogdetails.name)
    private readonly blogdetailModel: Model<Blogdetails>,
  ...
  private async createblogdetailindb(
    create: CreateBlogdetailRequest
  ): Promise<Blogdetails> {
    try {
      return this.blogdetailModel.create(create);
    } catch (error) {
      throw new BadRequestException(
        `Error while create blogdetail in database ${error.message}`
      );
    }
  }
  ...
  async findAll(): Promise<BlogDetailRespone[]> {
    try {
      const blogdetail = await this.blogdetailModel
        .find()
        .populate("comment")
        .exec();
      return blogdetail.map((blogdetails) =>
        this.mapblogdetailToResponse(blogdetails)
      );
    } catch (error) {
      throw error.message;
    }
  ...
  async findOne(id: string): Promise<BlogDetailRespone> {
    try {
      const check = await this.blogdetailModel
        .findById(id)
        .populate("comment")
        .exec();
      if (!check) {
  ...
  async update(
    id: string,
    updateblogdetail: UpdateBlogdetailRequest
  ): Promise<BlogDetailRespone> {
    try {
      const update = await this.blogdetailModel
        .findByIdAndUpdate(id, updateblogdetail, { new: true })
        .populate("comment")
        .exec();
      if (!update) {
  ...
  async updateComment(
    ...
    try {
      const blog = await this.blogdetailModel
        .findById(blogId)
        .populate("comment")
        .exec();
      ...
      const cmt = blog.comment.find(
        (comment) => comment._id.toString() === commentId
      );
      console.log("cmt>>", cmt);
      if (!cmt) {
        throw new NotFoundException(
          `Comment with ID ${commentId} not found in this blog`
        );
      }

      const updatedComment = await this.commentModule
        .findByIdAndUpdate(
          commentId,
          {
            $set: {
  ...
  async removeCommentFromBlog(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        `Error while removing comment: ${error.message}`
      );
    }
  }
