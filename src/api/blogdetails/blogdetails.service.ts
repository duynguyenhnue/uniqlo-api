...
export class BlogDetailService {
  ...
  try {
    // Tìm blog theo ID
    const blog = await this.blogdetailModel.findById(blogId).populate('comments').exec();
    if (!blog) {
      throw new NotFoundException(`Blog with ID ${blogId} not found`);
    }

    // Tìm index của comment cần cập nhật
    ...
    // Lấy comment cụ thể và cập nhật các trường mới
    const commentToUpdate = blog.comment[commentIndex];
    const updatedCommentData = {
      name: updateComment.name || commentToUpdate.name,
      email: updateComment.email || commentToUpdate.email,
      phone: updateComment.phone || commentToUpdate.phone,
      comment: updateComment.comment || commentToUpdate.comment,
    };
    ...
    if (!updatedComment) {
      throw new NotFoundException(`Failed to update comment with ID ${commentId}`);
    }

    // Cập nhật thành công, trả về toàn bộ blog với comment đã được cập nhật
    return this.mapblogdetailToResponse(
      await this.blogdetailModel.findById(blogId).populate('comment').exec(),
    );
  } catch (error) {
